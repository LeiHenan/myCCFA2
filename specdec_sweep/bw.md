...
> **NEW (Aug 2026): [DeepSeek-V4-Flash 284B on 2× RTX PRO 6000 — 150–170 tok/s](docs/DEEPSEEK-V4-FLASH.md).**
> Includes the two findings that cost us the most time and are documented nowhere else:
> **consumer Intel boards silently corrupt GPU↔GPU P2P** (every tool reports OK; a
> 60-second truth test catches it) and the model's no-think BOS-spam failure mode +
> the one-line `bad_words` fix. Also: we've since moved this box from WSL2 to
> **bare-metal Linux** — measured on the same hardware, Qwen3.6-27B decode went
> 92 → **117 tok/s** (+27%), 4-stream aggregate 232 → **377** (+63%), and the KV pool
> 204k → **944k tokens** (no WDDM VRAM reserve; the whole `--gpu-memory-utilization`
> cliff-dance below is a WSL-only phenomenon). The WSL guide below remains valid as
> written.
> Migration traps + full before/after tables: [docs/BARE-METAL.md](docs/BARE-METAL.md).

Qwen3.6 27B runs well on the RTX PRO 6000 Blackwell (96GB), but getting vLLM + MTP speculative decoding to actually work on sm_120 in WSL2 requires workarounds that aren't documented anywhere in one place. This is that guide — from a fresh WSL2 install to a working OpenAI-compatible API server. Tested on WSL2; bare-metal Linux measured +27% decode / +63% multi-stream (see banner above).
