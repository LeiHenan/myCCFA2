...
- **量级**：**L1 → 已量化（2026-09-13，零卡，decision #72）**：三个真实混合模型的**每序列状态 = 1.9–67 MiB**（Nemotron-H-8B **49.41 MiB** / Falcon-H1-7B **66.90 MiB** / Qwen3-Next-80B **37.69 MiB**）；与「一份 KV」的交叉点 **L\* = 1557–3162 tokens** ⇒ **bs=32、ctx=4096 时状态占每序列缓存内存 27.5%–43.6%**。另有一条硬约束：检查点开销/KV = `L*/b`（与 ctx 无关）⇒ 想要 **b=16（KV 块粒度）**的状态复用，代价是 **KV 本身的 97–198 倍**。结果：`results/aux-c3-hybrid-state/2026-09-13/summary.md`。
- **证据**：`mamba_cache_mode` 三态定义（本机源码）；状态大小可由模型 config 直接算出（层数 × `n_heads` × `head_dim` × `d_state` + conv state），**不用卡就能算占比**。
- **反证**：若状态总量相对 KV 可忽略（<1%）⇒ 不构成痛点。**该反证已被实测否定**（4k×bs32 下占 27.5–43.6%）；但**新的反证仍然成立**：这些内存若只是「占着」而不引发抢占 / 降并发，就**不产生吞吐损失** ⇒ 必须做取证第 2 步（≤1 GPU·h）才能定生死。
...
| **L1（已量化 2026-09-13）** | **P3**（状态占缓存 27.5–43.6% @4k×bs32；粒度约束 `b ≥ L*`） | **P3 可进入 S3**（但须先过取证第 2 步：确认它真的造成吞吐损失） |
| **L0**（尚无数字） | **P1、P2**、P6 | **P1/P2 不进入 S3，转 §取证任务（仍属 S1 的回环）** |
| 已被占 | P6、P7 | 否 |

> ⚠️ **进度（2026-09-13）**：**P3 已完成零卡量化（L0 → L1）**；**P1 / P2 仍是 L0** ⇒ 按判据不得进入 S3。下一步仍是取证：P1/P2 靠**第 2/3 步**（≤1 GPU·h），P3 需要**第 2 步**确认「内存占比高 ⇒ 真的损失吞吐」，否则它只是内存账、不是性能痛点。

## 取证任务

**目标**：把 P1 / P2 / P3 从 L0 抬到 L1（有数字）。**只测不治**（S1 阶段禁止提方案）。成本上限 **≤1 GPU·h**。

1. ✅ **零卡先算（已完成 2026-09-13，0 GPU·h）** —— 结论：**P3 未被淘汰，反而升级为 L1**（状态 1.9–67 MiB/序列；L\* = 1.6k–3.2k tokens；4k×bs32 下占比 27.5–43.6%；且 `检查点开销/KV = L*/b` ⇒ b 必须 ≥ L\*，否则缓存状态比缓存 KV 更贵）。副产品：**`bamba` 不在 vLLM 0.29 注册表内**（第二家族需换模型/换版本）；**`mamba_block_size` 默认值待确认**（疑似 = `max_model_len`）。脚本 `probes/aux-c3-hybrid-state/budget.py`，结果 `results/aux-c3-hybrid-state/2026-09-13/`。
