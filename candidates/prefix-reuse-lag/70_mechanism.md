...
## 插桩 v7（逐组探测）：**块在两轮里都齐了，复用被拒的原因不在缓存**

| lookup | `computed` | 9 组逐组探测 `h0_by_group` |
|---|---|---|
| #001（第 1 次） | 0 | `000000000` |
| **#033（第 2 次）** | **0** | **`111111110`** ← 组 0–7 全都有块 |
| #065（第 3 次） | 4080 | `111111110` |

**两条硬结论**：
1. **第 8 组永远没有该块**（第 2、3 次都一样）⇒ 它很可能**根本不参与前缀缓存查询**（例如某类不可缓存的状态组）；
2. **参与查询的组 0–7 在第 2 次就已经全部有块**，而 `computed` 仍是 **0**；第 3 次探测状态**完全相同**（`111111110`）却返回 **4080**。

⇒ **decision #88 的"全组一致导致归零"被推翻**：组没问题、块没问题、哈希没问题。
**复用的拒绝发生在缓存查找之后** —— 即调度器/分配器一侧（例如在投机路径下决定不使用已计算块，
或 `allocate_slots` 在该状态下回退到重算）。

## 本线索结案（decision #93）

**结案性质**：**刻画 + 机制边界**，不是优化。
- **现象（稳固）**：投机解码把重复前缀的复用生效点从第 2 次推迟到第 3 次，代价 **2.09×（投机关）/ 2.63×（投机开）**，
  TTFT 差 −57.5%~−90.8%；四格复现，噪声 ≤2%。
- **机制（已定位到边界）**：**不在缓存侧**（块齐、哈希命中、无驱逐、无对齐取整、非 EAGLE 丢块、非组耦合），
  **在缓存查找之后的调度/分配决策**。
- **8 条被排除的假设**（逐条有实验或源码依据，见上文各节）。
- **为什么不再往下做**：剩下的路径需要改写调度器在投机下的"是否采用已计算块"的决策 —— 引擎手术量级，
  超出本目标"决策级验证"的半径；**且我三次干预均未撼动 onset**，符合预登记的降级条件。
