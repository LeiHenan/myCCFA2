...
# P 阶段（同日晚追加）：新副本语料获取策略 —— **整机账杀死热路径主张**

**预登记**：`notes/prereg/sglang-draft-corpus-p.md`（采数前冻结，commit `9668fdf`）｜**成本 ≈0.25 GPU·h**
**原始数据**：`/root/ccfa_results/2026-09-13/p12_policy/`｜**判定**：P1 ✅ / **P2 ✅** / P3 ❌ / **P4 ❌ ⇒ 杀策略方向**

| 臂 | 首通 accept | 目标(s) | 准备(s) | 整机总时(s) | vs 从 0 学 |
|---|---|---|---|---|---|
| `S_cold` 从 0 学 | 1.906 | 24.468 | 0 | **24.468** | — |
| `S_bare` 无前缀目标续写 | 3.064 | 19.820 | 24.881 | 44.702 | −82.7% |
| `S_peer` **异租户**续写（共享前缀） | **3.064** | 19.960 | 24.878 | 44.839 | −83.3% |
| `S_text` 目标 prompt 原文 | **1.579** | 28.735 | 0 | 28.735 | −17.4% |
| `S_self` 目标分布自身续写 | **6.087** | **11.386** | 24.554 | **35.940** | −46.9% |

**关键派生**：盈亏平衡 `k* = C_prep / (t_cold − t_warm) = 24.554 / (24.468 − 11.386) = ` **1.87 遍**
⇒ 预热只在"同一副本服务同一 workload ≥2–3 遍"时回本，而该区间由**引擎自带会话内累积**覆盖 ⇒ 净收益 ≈ 0。

**机制派生**：`S_peer = S_bare = 3.064`（逐位）⇒ `max_trie_depth=18` 把 512-token 共享前缀挡在草稿窗口外；
`S_text < S_cold`（1.579 < 1.906）⇒ 发送原文**主动伤害**接受长度。

**结论**：本候选**不作为论文立项**（预登记 P4 杀判据触发）。保留四条可迁移结论与三阶段实测数据。
**未测**：多副本并发继承、跨机传输、前缀 <18 token、并发 >1、其他模型/上下文长度。
