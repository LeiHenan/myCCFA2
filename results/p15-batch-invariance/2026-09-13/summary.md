...
## 六、第 8 轮：**跨引擎有效性对照（vLLM 在 sm120 长序列上也被证实）** + 合批的墙钟证据

**动机**：issue 报告的发散都在 **sm_90/FA3** 或高共驻（#51187 ~44 seqs），**没有人报 sm120 长序列**；
而 vLLM 的 `envs.py` 注释声称需要 **sm ≥ 9.0**（docs 说 8.0+）⇒ **sm120 属"文档声称支持"的算力**。这是唯一只有本机能填的空格。

**设置**：vLLM 0.29.0 / Qwen3-4B / sm120 / `--enforce-eager --no-enable-prefix-caching`
/ prompt = **4096 token** / `max_new_tokens=64` / **真贪心**（`temperature=0`，vLLM 侧即贪心）/ 每格 2 次重复。

| 设置 | n=1 | n=8（并发相同请求） | wall |
|---|---|---|---|
| **BI=0（默认）** | unique 1/1, div 0.0000 | **unique 3/8, div 0.5000** | 0.90 → 1.81 s（**2.0×**） |
| **BI=1** | unique 1/1, div 0.0000 | **unique 1/8, div 0.0000** | 2.08 → 3.02 s（**1.45×**） |
| `--max-num-seqs` 8 vs 32 | — | 结论**相同**（BI=0 均 div 0.5；BI=1 均 0.0） | — |

**三条结论**：

1. **vLLM 的开关在 sm120 + 长序列下有效**：默认配置 8 个并发相同请求 → **3 个不同结果**（div 0.5）；
   开 `VLLM_BATCH_INVARIANT=1` 后 → **1 个**（div 0.0）。⇒ **首次在这个算力上给出前后对照**（此前公开报告集中在 sm_90/FA3）。
2. **合批的独立证据（补上前几轮的缺口）**：n=8 的墙钟只有 n=1 的 **2.0×**，而**若 8 个请求串行执行该是 8×**
   ⇒ **8 个请求确实同批执行**。这条证据此前一直缺（第 6 轮因此把结论降级过）。
3. **代价 1.7×**（n=8：1.81 → 3.02 s），与 SGLang 的 **1.45–3.6×** 同量级。

**跨引擎汇总（本机实测，sm120 / 4096-token prompt / 64 token / 真贪心）**：

| 引擎 | 默认发散 | 开开关后 | 代价（wall） |
|---|---|---|---|
| **vLLM 0.29.0** | **3/8 不同**（div 0.5） | **1/8**（div 0.0） | **1.7×** |
| **SGLang 0.5.19** | **8/8 不同**（`unique_fp=8`，div_text 0.25） | **1/8**（div 0.0） | **1.45–3.6×** |

**"选择性恢复"再评估（第 8 轮新增）**：BI 的替换是 `enable_batch_invariant_mode()` 里的**一批 Python 级算子替换**
（`determinism/batch_invariant.py` 内 23 个函数：`mm/matmul/bmm/addmm/softmax/log_softmax/mean/rms_norm/linear`）
⇒ **原理上可按算子选择性启用**，形式上存在做"更省方案"的空间。
**但我没有做，理由要写清**：逐算子验证需要 **8 个算子 × 一次补丁 + 一次实验 ≈ 1–2 GPU·h**，
而**引擎团队选择全关**这一事实本身就是"必要集合难以缩小"的先验证据；
按本工作区判据，"**没有证据表明存在更省的等价方案**"不能靠"形式上有空间"来替代 ⇒ **不做**。
**若将来要做**：这才是把"可回收性"补上的唯一路径（需先做 S4 oracle 上界：完美地只关必要算子能省回多少）。
