...
| **备线 A** `p04` | **#4 收窄版**：多分支 SSM 快照内存放大 + checkpoint 淘汰 | 提交语义半边已 MERGED/OPEN；**内存与淘汰半边空** | 1–2 卡 / 2 周 | 引擎常量（512 KB/层；每序列仅 1–2 checkpoint） | §6（先过复核门） |
| **备线 B** `p12` | **#12 MoE EP 不均衡 + dispatch** | 拥挤（DA-MoE 等） | 2 周（需多卡 EP） | **实测最硬**（真实 trace：skew 5–80×；3.28× 层延迟） | §7（2 周时间盒） |
| **备线 C** `p07`（待判定） | **#7 per-adapter KV 配额/准入** | 空（两引擎均无 KV 配额/分区；`#2929` 的 21 项全是适配器权重/算子/API） | ≤1 周 / 1 卡 | 算术 1.2–2×（**③ 未过，待换实测**） | §8.4（离散度 <5pp ⇒ 归档） |
...
- **判据**：**离散度 < 5 个百分点 ⇒ 归档 #7**；≥ 5 且存在"高占用低命中"adapter ⇒ 升为备线 C。
...
> 20 个候选 = **14 归档 + 6 存活**（`#1 #3 #4 #6 #7 #12`）。
