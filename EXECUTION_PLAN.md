...
**当前阶段（2026-09-12 更新）**：✅ 新机器已就位并通过验收（RTX PRO 6000 Blackwell 96 GB / 驱动 580.82.09 / vLLM 0.29.0）。判定实验已跑完，但 **`#6` 的结论须降级为「条件性阴性（coverage-limited）」，不得当作已成立的 No-Go**：
- **T0 = 结局 A**（深度旋钮可操作：接受长度 1.303→2.829、tok/s 101.5→180.3 单调）；
- **T1 = 斜 ridge**（γ* 随深度 3→7，严格正交性被否证）⇒ 转 H1b；
- **T2 = oracle 上界增量 +0.1%**（4k +0.00% / 32k +0.30%）—— **但只在 `bs=1`、`ctx∈{4k,32k}` 的子空间上算得**；
- **E1 = `#1` 判死**（`R_byte` p95 **0.072%** ≪ 5%，12 格，两次独立运行逐位一致）—— 这条**成立**；
- **D3「消除保守预留」= 关闭**：原"触发"经查是**指标定义假信号**（`allocate_slots` 返回的是**新增**块，故旧比值恒为 `block_size/γ`）；按正当口径（请求持有总块 ÷ 已提交）实测 = **1.0053** ⇒ 只多占 0.53% ⇒ **无 headroom，不立项**（decision #51）—— 这条**成立**。
- ⚠️ **`#6` 未排除的三处覆盖缺口（2026-09-12 复核，decision #53）**：① **只测了 `bs=1`** —— 而 prereg 明写在反转位置不确定时**必须扩到 `bs{1,8,32}` 再判、不得据 T1 阴性直接杀**；② **长上下文只到 32k**（Qwen3-4B 上限 40960），而该方向的动机（DFlash 185k 全上下文重扫代价）**恰好在未测区间**；③ γ 只到 7（对 DFlash2 已是架构上限，但**换 block 更大的族可测更长 γ**）；另需补一个**已发布浅层 checkpoint** 对照，以排除"截断只给下界"的解释。
⇒ **下一步**：按 §12 D1 条款在**同一预登记下扩 bs**（强制项）+ 换 target 补长上下文 + 换族补长 γ，跑完再判。
...
- **🔴 实测结论（2026-09-12，`#6` 结题）**：
  ...
  - ~~**角点占优**~~ **【2026-09-12 已被 S1 证伪，见 decision #61】**：原以为"深度单调有利"，但 ① T1 在 `ctx=32768, γ∈{1,3}` 上 **d3 > d5**（33.3 vs 31.4 / 29.4 vs 27.0）；② **S1 在 `ctx=4096, bs∈{8,32}` 上彻底翻转：d3 > d1 > d5**（bs=8: 656 vs 496 vs 407；bs=32: **1081 vs 862 vs 472**），且这些格 5 次重复极差仅 1.5–3.1%，bs=32 三档 `completed=32/32`、d5 的 TTFT 3705 ms 对 d3 的 510 ms ⇒ **不是显存/排队假象**。
  - ⇒ **深度最优值依赖运行点**：bs=1 要最深、bs≥8 要中等深度。这是**非退化的深度选择问题**，正是 `#6` 假设的"算力分配"现象 ⇒ **`#6` 由条件性阴性转为「有实质证据支持」**（待 S1 完整网格确认）。
  ...
  - 完整论证：`results/p06-frontier/2026-09-12/T2-verdict.md`。**判死前必须先补**：S1（bs 扩展，进行中）、S2（≥128k 上下文，需 ≥48 GB 显存）、S3（换族测长 γ）、S4（shipped-adapter 基线）。
