...
**当前阶段（2026-09-12 更新）**：✅ 新机器已就位并通过验收（RTX PRO 6000 Blackwell 96 GB / 驱动 580.82.09 / vLLM 0.29.0）。**判定实验全部跑完，两条线都由实测关闭**：
- **T0 = 结局 A**（深度旋钮可操作：接受长度 1.303→2.829、tok/s 101.5→180.3 单调）；
- **T1 = 斜 ridge**（γ* 随深度 3→7，严格正交性被否证）→ 转 H1b；
- **T2 = 增量 +0.1%**（≪ 8% 门槛）⇒ **`#6` No-Go**；γ 可行上限实测 = 7（= 训练操作点），边界疑点解除。
- **E1 = `#1` 判死**（`R_byte` p95 **0.072%** ≪ 5%，两次独立运行逐位一致）；
- **D3「消除保守预留」= 关闭**：原"触发"经查是**指标定义假信号**（`allocate_slots` 返回的是**新增**块，故旧比值恒为 `block_size/γ`）；按正当口径（请求持有总块 ÷ 已提交）实测 = **1.0053** ⇒ 只多占 0.53% ⇒ **无 headroom，不立项**（decision #51）。
⇒ **下一步见 `notes/decision_log.md` #47–#52**：主线空缺（`#12` 冻结待 StreamEP、`#7` 仅算术量级、`#3` 先验 ≤6%、`#4` 暂缓）。续做只能按 §12 **D1 条款第 3 条**启动**新的独立预登记**，或重排 portfolio；**不得**把本轮数据重新解读成测量论文。两份结题文档：`results/p06-frontier/2026-09-12/T2-verdict.md`、`results/p01-e1-uncommitted-kv/2026-09-12/VERDICT.md`。
