...
## G. Cross-engine abandonments and the negative-result tail (C-section expansion)

### G1. llama.cpp / Ollama / TGI / TensorRT-LLM — draft heads abandoned or proven slower

| # | What was tried | Outcome | Verbatim | URL | Read |
|---|---|---|---|---|---|
| G1.1 | MTP (nextn) head for Qwen3.8-Flash-Next | **closed-unmerged** | ggml-gh-bot: *"AI-generated content : While code is allowed to be generated by AI, please write the PR description and commit messages on your own without the help of AI."* | https://github.com/ggml-org/llama.cpp/pull/27842 | READ BODY |
| G1.2 | SYCL MTP on Intel Arc | **not planned**; **21% SLOWER at 100% draft acceptance** | *"**MTP is ~21% slower than generating without speculation, despite 100% draft accuracy.** This is the opposite of the expected result (typically 1.5-2x speedup on CUDA)."* | https://github.com/ggml-org/llama.cpp/issues/23533 | READ BODY |
| G1.3 | Gemma 4 MTP merge (#23398) | **not planned**; 40 tok/s → ~4 tok/s | *"#23398 When I use gemma 4 12B on this new merge my tokens/sec **drop to ~4 tokens/sec, previously I was getting 40+ tokens/sec.**"* | https://github.com/ggml-org/llama.cpp/issues/24266 | READ BODY |
| G1.4 | Pipelining `draft-mtp` → `ngram-mod` | **not planned** + stale; no speedup | *"Adding ngram-mod independently on top provides **no speedup, only verification overhead.**"* | https://github.com/ggml-org/llama.cpp/issues/23184 | READ BODY |
| G1.5 | DFlash in Ollama (`muse-glimmer:30b-nvfp4-dflash`) | **not planned**; user measured no effect | *"I'm able to get 25 tps which is respectable and on par with Meta video on their release blog … without the speculative decoding. This indicates to me that the `muse-glimmer:30b-nvfp4-dflash` **doesn't really use the DFlash speculative decoding.**"* | https://github.com/ollama/ollama/issues/17683 | READ BODY |
| G1.6 | Two-model draft/verify in HF TGI | **not planned** + Stale (2023, canonical) | *"Adding this feature would require making TGI more generic, so that one can run multiple models at once. We would need to make sure that this does not degrade performance or reliability for the single model use case."* | https://github.com/huggingface/text-generation-inference/issues/729 | READ BODY |
| G1.7 | Assisted Generation in TGI | **not planned** + Stale | — | https://github.com/huggingface/text-generation-inference/issues/314 | READ BODY |
| G1.8 | Medusa acceleration in TGI | **not planned**; "acceleration in doubt" | *"Speculative(3) seems to accelerate significantly, but I didn't observe the same result with vLLM benchmark and in real use on the same A30*4. **Their results look like Medusa doesn't have that much effect.**"* | https://github.com/huggingface/text-generation-inference/issues/1503 | READ BODY |
| G1.9 | EAGLE-3 acceptance length at larger batch | **not planned**; negative scaling | issue title: *"**Eagle3 acc len decreases with bigger batch size**"* | https://github.com/NVIDIA/TensorRT-LLM/issues/9208 | READ BODY |
| G1.10 | EAGLE-3 one-model config + FP8 target | **not planned** | *"The Support Matrix of the Eagle example README suggests that **FP8/INT8 are not supported.**"* | https://github.com/NVIDIA/TensorRT-LLM/issues/7842 | READ BODY |
| G1.11 | Draft-cache replay `p_min`; external DFlash temp-0 invariant; Gemma E2B draft; spec-context memory fitting; SYCL `draft-mtp` memory | all **not planned** | — | https://github.com/ggml-org/llama.cpp/issues/26100 · https://github.com/ggml-org/llama.cpp/issues/27975 · https://github.com/ggml-org/llama.cpp/issues/22337 · https://github.com/ggml-org/llama.cpp/issues/25408 · https://github.com/ggml-org/llama.cpp/issues/23203 | TITLE ONLY |
| G1.12 | MTP drafter under rejection (GGUF/Metal, issue **still Open**) | 2.3× slower than no speculation | *"When drafts are rejected, mtp-q4_K_M falls to 5.14 tok/s, i.e. **2.3x slower than no speculation at all. The draft alone therefore costs slightly more than a full pass.**"* | https://github.com/ollama/ollama/issues/17776 | READ BODY |

### G2. vLLM — the negative-result tail (all closed `not planned`)

| # | What was tried | Outcome | Verbatim | URL | Read |
|---|---|---|---|---|---|
| G2.1 | **Logits-distilled** small draft with *"very high"* acceptance | **not planned**; ~30% **slowdown** | *"I trained the small model using **logits distillation** of main model, so it has a good level of generation (**acceptance rate is very high**) Still I get **consistent performance drop ~30%** in terms of speed when using 5 speculative tokens… **the best speed is achieved when using main model only without speculative.**"* | https://github.com/vllm-project/vllm/issues/15025 | READ BODY |
| G2.2 | Small Qwen3 draft for Qwen3-32B-FP8 | **not planned**; 13–14 → 10–11 tok/s | *"But it turns out that it leads to a **slower inference speed** (around 10-11 tokens/s). I don't know if it's normal."* | https://github.com/vllm-project/vllm/issues/21278 | READ BODY |
| G2.3 | Multimodal external drafters | **not planned** + stale | benchislett: *"Currently, multimodal EAGLE drafters are supported, but only for EAGLE drafters and only when not using parallel drafting. This is due to some complexity in managing the MRoPe position embeddings and multimodal state that **has not been explored for external drafters.**"* | https://github.com/vllm-project/vllm/issues/33458 | READ BODY |
| G2.4 | Speculative Speculative Decoding (async overlap) | **not planned** + stale | — | https://github.com/vllm-project/vllm/issues/36037 | READ BODY |
| G2.5 | Tree speculative decode | **not planned** + stale | *"I was testing tree speculative decoding and noticed that this feature is not yet fully implemented. **It also doesn't appear to be on the current roadmap.**"* | https://github.com/vllm-project/vllm/issues/37396 | READ BODY |
| G2.6 | n-gram + suffix in `model_runner_v2` | **not planned** | — | https://github.com/vllm-project/vllm/issues/38069 | READ BODY |
| G2.7 | "Tracking Spec Decode Support" matrix | **not planned** (tracking abandoned) | — | https://github.com/vllm-project/vllm/issues/27691 | READ BODY |
| G2.8 | CI/CD regression testing for spec decode | **not planned** + stale | — | https://github.com/vllm-project/vllm/issues/28135 | READ BODY |
| G2.9 | Draft model with shorter context than target | **not planned** | error: *"Cannot handle cases where distributed draft workers generate no tokens"* (target 128K ctx, draft 32K ctx) | https://github.com/vllm-project/vllm/issues/7859 | READ BODY |
| G2.10 | GPT-OSS 20B as drafter · EAGLE-3 draft len > 2048 · EAGLE-3 second-token acceptance collapse · draft/target hidden-size mismatch | all **not planned** | — | https://github.com/vllm-project/vllm/issues/33133 · https://github.com/vllm-project/vllm/issues/23072 · https://github.com/vllm-project/vllm/issues/33330 · https://github.com/vllm-project/vllm/issues/37966 | TITLE ONLY |
| G2.11 | Medusa choice tree · Medusa TP>1 hang · Medusa concurrency 2 · Medusa vs baseline · "Automate SD" RFC · Speculative Streaming · per-sequence spec decode · second-device draft · "why is spec decoding slower" | all **not planned** | — | https://github.com/vllm-project/vllm/issues/20813 · /16477 · /10031 · /6777 · /4565 · /2943 · /17984 · /12200 · /8439 | TITLE ONLY |
| G2.12 | `speculators` scope narrowings (non-EAGLE drafts, multi-node, Ulysses SP, DFLASH windowed-attention backward) | all **not planned** | #1105: *"At production sizes **the dense backward needs ~69GB (OOM on 8×A800)** and ignores the window structure."* | https://github.com/vllm-project/speculators/issues/231 · https://github.com/vllm-project/speculators/issues/356 · https://github.com/vllm-project/speculators/issues/338 · https://github.com/vllm-project/speculators/issues/1105 | READ BODY |
| G2.13 | `speculators` hybrid training mode | **removed as a breaking change** | *"**Removes hybrid training mode (breaking)** --on-generate cache kept freshly generated hidden states so later epochs could reuse them. **It was broken on the Mooncake backend**… **every generated sample leaked into the store while nothing was ever cached** — MooncakeTransfer.get_cached() returns None unconditionally, so **each epoch regenerated everything from scratch**."* | https://github.com/vllm-project/speculators/pull/1036 | READ BODY |
| G2.14 | Backbone/TTT-internal position trimming (SpecForge) | **not planned** | *"Are maintainers open to backbone-internal position trimming in the online TTT loop (behind a default-off flag), or is loss-end trimming the preferred boundary?"* | https://github.com/sgl-project/SpecForge/issues/706 | READ BODY |

### G3. Community negative results on the shipped DFlash/DFlash2 drafters

| # | Finding, verbatim | URL | Read |
|---|---|---|---|
| G3.1 | *"**Both DFlash (3.6) and DFlash 2 (3.8) are slower than MTP on the 2 machines that I've tested.** … **DFlash gains seem very not true.**"* / *"**In reality though performance uplift is very uneven.**"* / *"**My only issue is that it uses more vram than MTP.**"* | https://huggingface.co/z-lab/Qwen3.8-27B-DFlash2/discussions/3 | READ BODY |
| G3.2 | *"DFlash v1 (n=5) **decays to 1.02x by 32k, i.e. by then it is worth nothing over plain decode.**"* / *"it seems to be **not significantly better than MTP**"* (RTX 3090: MTP 57.79 t/s / accept 0.6317 vs DFlash2 62.21 t/s / 0.6153) | https://github.com/ggml-org/llama.cpp/pull/27342 | READ BODY |
| G3.3 | *"the current `qwen3-8b-dta.json` configuration seems to **silently fall back to regular DFlash training** instead of enabling `vp_drafter`"* (VP-Drafter logic removed in-tree) | https://github.com/sgl-project/SpecForge/issues/679 | READ BODY |
| G3.4 | *"**TTT implementation is not the original EAGLE-3 paper's concept, but merely data augmentation.**"* (dispute never formally resolved) | https://github.com/sgl-project/SpecForge/issues/227 | READ BODY |

### G4. Papers that tried it and reported it does not work (additional to C8)

| # | Try → outcome, verbatim | URL | Read |
|---|---|---|---|
| G4.1 | PEFT-BD (LoRA-style adapter as block-diffusion drafter on the *same* backbone) → **self-described negative result**: *"Despite these advantages, **PEFT-BD does not yield a practical speedup in our Qwen3-0.6B experiments.**"* / *"**the drafter is parameter-efficient but not compute-efficient.**"* | https://arxiv.org/abs/2607.12422 | READ BODY |
| G4.2 | Test-time training as the fix for long-range draft decay → **decay persists**: *"Existing work attributes this decay to train-inference mismatch and proposes test-time training (TTT) as a remedy, yet **we observe that long-range decay persists even in TTT-trained drafters.**"* | https://arxiv.org/abs/2604.26412 | READ BODY |
| G4.3 | Block-parallel/diffusion drafters on **multimodal** targets → **below baseline**: *"At 8K, all settings fall to or below the autoregressive baseline, with speedups of **0.90× and 0.83×**"* / *"on the Qwen3-VL family, **EAGLE-3 fails to provide speedup on SGLang (0.71× for 4B and 0.88× for 8B).**"* | https://arxiv.org/abs/2608.20743 | READ BODY |
| G4.4 | Full-parameter online draft alignment → *"Online alignment improves draft quality, but **full-parameter updates introduce substantial memory and latency overhead.**"* | https://arxiv.org/abs/2605.27390 | READ BODY |
| G4.5 | Serving-time drafter adaptation at short generation lengths → **declined to evaluate**: *"**HumanEval at 164 prompts and MT-Bench at 80 prompts are too short for the acceptance gain to amortize the backward cost of adaptation, so we do not evaluate adapt on them.**"* | https://arxiv.org/abs/2605.07243 | READ BODY |
| G4.6 | Larger-scale draft training (DFlare) → **resource-limited**: *"the training cost of DFlare is high due to the large draft model and the scaled training corpus… **we were unable to explore this direction due to computational resource constraints.**"* | https://arxiv.org/abs/2606.02091 | READ BODY |
| G4.7 | MLA conversion for drafts → *"direct MHA/GQA-to-MLA conversion can sharply reduce this agreement… may be tolerable for standalone generation but substantially lower draft-token acceptance"* | https://arxiv.org/abs/2607.27269 | READ BODY |

### G5. Draft-model quantization — **counter-evidence that bounds the "quantization breaks the head" claim**

Section C is full of quantized-head failures. These are the rows that **worked**; they bound the failure claim rather than contradict it.

| # | Finding | Verbatim | URL | Read |
|---|---|---|---|---|
| G5.1 | **The draft quant need not match the target's quant** | *"Q8 DFlash2 works fine with a Q4 target — **the draft quant does not need to match the target**"*; RTX 3090: no-spec 33.16 → MTP2 48.32 → DFlash2 Q4 56.06 → DFlash2 **Q8 59.88** t/s | https://huggingface.co/z-lab/Qwen3.8-27B-DFlash2/discussions/9 | READ BODY |
| G5.2 | **A quantized target does not inherently break an EAGLE3 draft** | Kimi-K2.5 **W4A8** + EAGLE3: TPOT 42.73 → 27.41 ms (−35.9%), throughput 672 → 895 tok/s (+33.1%), *"with no measurable accuracy regression"* | https://rocm.blogs.amd.com/artificial-intelligence/kimi-k2.5-speculative/README.html | READ BODY |
| G5.3 | DFlash + **FP8-quantized verifier** compounds positively | Speculators v0.5.0: *"Combining DFlash with an FP8 quantized verifier yields even greater gains"* | https://vllm.ai/blog/2026-05-28-speculators-v050 | READ BODY |
| G5.4 | **Reduced draft vocab is a supported training flag** | Speculators: `--draft-vocab-size`, `--token-freq-path`, `--t2d-path`, `--d2t-path` | https://raw.githubusercontent.com/vllm-project/speculators/main/docs/cli/train.md | READ BODY |
| G5.5 | **Vocab-pruned EAGLE3 draft in the wild** | `MiniMax-M2.7-EAGLE3-draft-vocab32k`: target vocab 200064, draft vocab 32000, trimmed `[32000, 3072]` head 6.25× smaller; HumanEval mean accept length 2.59 | https://huggingface.co/asherszhang/MiniMax-M2.7-EAGLE3-draft-vocab32k | READ BODY |
| G5.6 | **4-bit claimed as the draft sweet spot** | *"experimentation has shown using more or less than 4-bits for speculative decoding is a waste of time anyway"*; also *"The 14 heads of `Qwen2.5-0.5B` doesn't allow for any of the other 4-bit quants to be made"* | https://huggingface.co/jukofyork/DeepSeek-R1-DRAFT-0.6B-v3.0-GGUF/blob/main/README.md | READ BODY |
| G5.7 | **Low-rank LM head beats vocab truncation** | SlimSpec: *"4-5× acceleration over the standard LM-head architecture while maintaining a competitive acceptance length"*, vs vocab truncation which *"reduce[s] LM-head latency by only about 60%"* | https://arxiv.org/abs/2605.10453 | READ BODY |
| G5.8 | Reduced draft vocab + unconditional lm_head sharing **broke** DFlash drafts | vLLM-Ascend PR #9834: `_maybe_share_lm_head` *"unconditionally replaced the draft model's lm_head with the target model's lm_head whenever method was in `(\"eagle\", \"dflash\")`"* — broke `RedHatAI/Qwen3-8B-speculator.dflash` with `draft_vocab_size=32000` | https://github.com/vllm-project/vllm-ascend/pull/9834 | READ BODY |
| G5.9 | **Vocabulary speculation instead of reduction** | SpecVocab: *"up to an 8.1% increase in average throughput over EAGLE-3"* (ACL 2026 Findings) | https://arxiv.org/abs/2602.13836 | READ BODY |

### G6. NOT hardware-ruled-out on 1× RTX PRO 6000 96 GB / sm120 — corrects an over-broad reading of section D

| # | Item | Evidence | URL | Read |
|---|---|---|---|---|
| G6.1 | **vLLM standalone draft-model spec decode** | PR #24322's own benchmark rig is *"an RTX PRO 6000 96GB"* — the same GPU class | https://github.com/vllm-project/vllm/pull/24322 | READ BODY `[v]` |
| G6.2 | **Speculators draft training** | CLI: *"Supports **single-GPU** and multi-GPU distributed training"* | https://raw.githubusercontent.com/vllm-project/speculators/main/docs/cli/train.md | READ BODY |
| G6.3 | **PARD-style parallel drafting** | docs give a `-tp 1`, `--gpu-memory-utilization 0.8` single-GPU online command | https://raw.githubusercontent.com/vllm-project/vllm/main/docs/features/speculative_decoding/parallel_draft_model.md | READ BODY `[v]` |
| G6.4 | **SGLang TLI cross-vocabulary drafts** | benchmarked on **single** H100 NVL 96 GB, A100 80 GB, RTX A6000 48 GB | https://github.com/sgl-project/sglang/pull/22883 | READ BODY |
| G6.5 | **llama.cpp MTP draft-vocab trimming** | measured on **a single RTX 5090 Laptop**; draft LM-head kernel −84.9% (621 µs → 91 µs/call) | https://github.com/ggml-org/llama.cpp/issues/25187 | READ BODY |
| G6.6 | **Standalone 0.5B drafter training** | *"~2.5 h on a **single 24 GB GPU**, ~$3 of compute"* | https://huggingface.co/vexp-ai/horizon-draft-0.5b | READ BODY |
| G6.7 | **Qwen3-4B EAGLE3 draft training** (weights published) | 1 layer, hidden 2560, draft vocab 32000, bf16, SpecForge online run — **this is this rig's target model** | https://huggingface.co/huluhuluu/Qwen3-4B-Instruct-2507-EAGLE3-ShareGPT-full-context-epoch1-step35000 | READ BODY |
| G6.8 | **DFlash2 GGUF draft for Qwen3.8-27B** | **1.92 B params** bf16 (~3.8 GB) — the draft side is small; the *target* is the memory question | https://huggingface.co/Akicou/Qwen3.8-27B-DFlash2-GGUF | READ BODY |

---

## H. N-gram / prompt-lookup / retrieval drafting — additional rows

### H1. CLOSED (shipped / published), additional to A2

| # | Question it answers | Who closed it | Artifact | URL | Status | Read |
|---|---|---|---|---|---|---|
| H1.1 | n-gram in vLLM V1 (no draft model) | LiuXiaoxuanPKU | PR #12193, merge `80f63a3` | https://github.com/vllm-project/vllm/pull/12193 | merged | READ BODY |
| H1.2 | Suffix-tree drafting in vLLM | aurickq / Snowflake | PR #25784 | https://github.com/vllm-project/vllm/pull/25784 | merged | READ BODY |
| H1.3 | n-gram on **GPU** + async scheduler | vLLM | PR #29184 (`ngram_gpu`) | https://github.com/vllm-project/vllm/pull/29184 | merged | READ BODY |
| H1.4 | Auto-disable SD at large batch | vLLM V0 | PR #4592 `--speculative-disable-queue-size`, merge `f942efb` | https://github.com/vllm-project/vllm/pull/4592 | merged | READ BODY |
| H1.5 | SGLang lookahead → NGRAM | SGLang | PR #11010, merge `24f7cb1` | https://github.com/sgl-project/sglang/pull/11010 | merged | READ BODY |
| H1.6 | External corpus + suffix automaton (SAM) in SGLang | SGLang | PRs #21425, #22203, #22294, #22471, #22538, #22737 | https://github.com/sgl-project/sglang/issues/21052 | merged | READ BODY |
| H1.7 | NGRAM + DP attention in SGLang | whycoming | PR #28616 — **removes** `ValueError("Currently ngram speculative decoding does not support dp attention.")`; validated 2×H20 tp2/dp2 | https://github.com/sgl-project/sglang/pull/28616 | merged | READ BODY |
| H1.8 | n-gram drafting in llama.cpp | ggml-org | `ngram-cache`, `ngram-simple`, `ngram-map-k`, `ngram-map-k4v`, `ngram-mod`; `--spec-default` enables `ngram-mod` | https://github.com/ggml-org/llama.cpp/blob/master/docs/speculative.md | shipped | READ BODY |
| H1.9 | Self-speculation from token history | srogmann | PR #18471 `--spec-self`; 181.24 → 445.75 tok/s, acceptance 0.76827 | https://github.com/ggml-org/llama.cpp/pull/18471 | merged | READ BODY |
| H1.10 | N-gram auto-enable heuristic | NVIDIA TRT-LLM | tech blog → `spec_decode_algo=AUTO` (commit `4e69c14`, updated 2026-04-08) | https://github.com/NVIDIA/TensorRT-LLM/blob/a4349b2cd9a521af85335a517722f90fc14d4f15/blogs/tech_blog/blog07_NGram_performance_Analysis_And_Auto_Enablement.html | shipped | READ BODY |
| H1.11 | REST | He et al. | arXiv 2311.08252, NAACL 2024 | https://arxiv.org/abs/2311.08252 | published | READ BODY |
| H1.12 | SuffixDecoding | Oliaro, Jia, Campos, Qiao | arXiv 2411.04975v3, **NeurIPS 2025 Spotlight** | https://arxiv.org/abs/2411.04975 | published | READ BODY |
| H1.13 | SAM Decoding | Hu et al. | ACL 2025 Long, pp. 12187–12204 | https://aclanthology.org/2025.acl-long.595/ | published | READ BODY |
| H1.14 | Token Recycling | Luo et al. | ACL 2025 Long, pp. 6816–6831, **Outstanding Paper** | https://aclanthology.org/2025.acl-long.338/ | published | READ BODY |
| H1.15 | DReSD (dense retrieval drafting) | Gritta et al. | Findings of ACL 2025, pp. 19822–19832 | https://aclanthology.org/2025.findings-acl.1017/ | published | READ BODY |
| H1.16 | RACER (retrieval + logits) | Zhang et al. | arXiv 2604.14885, Findings of ACL 2026; into SGLang via PR #39211 | https://arxiv.org/abs/2604.14885 · https://github.com/sgl-project/sglang/pull/39211 | published / PR open | READ BODY |
| H1.17 | Anisotropic trees for mixed-acceptance sources | Jin, Nguyen, Inoue | Goose, arXiv 2604.02047v2 | https://arxiv.org/abs/2604.02047 | published | READ BODY |
| H1.18 | Lookahead decoding | Fu et al. | arXiv 2402.02057 | https://arxiv.org/abs/2402.02057 | published | READ BODY |
| H1.19 | Canonical PLD repo | Saxena | `apoorvumang/prompt-lookup-decoding` | https://github.com/apoorvumang/prompt-lookup-decoding | shipped | READ BODY |
| H1.20 | Stochastic adaptive n-gram drafting for reasoning | STAND | arXiv 2506.04708 | https://arxiv.org/abs/2506.04708 | published | READ BODY |
| H1.21 | Learning-free **batched** n-gram speculation | Stewart et al. | N-Grammys, arXiv 2411.03786 (ENLSP-IV @ NeurIPS 2024) | https://arxiv.org/abs/2411.03786 | published | READ BODY |
| H1.22 | n-gram/suffix in Rust with longest-suffix proposer | danielwinterw | Mesh-LLM PR #1037, merge `88f8b95` (2026-07-22) | https://github.com/Mesh-LLM/mesh-llm/pull/1037 | merged | READ BODY |
| H1.23 | LRU n-gram cache, tables only | Ma et al. | Cacheback, arXiv 2511.21699, EMNLP 2025 main, pp. 31067–31072 | https://arxiv.org/abs/2511.21699 | published | READ BODY |
| H1.24 | Adaptive semantic + lexical model-free PLD | Liu et al. | AdaPLD, arXiv 2606.05742v2 | https://arxiv.org/abs/2606.05742 | published | READ BODY |
| H1.25 | Semantic retrieval + soft-gated verification | Chen et al. | SENSE, arXiv 2606.00021 | https://arxiv.org/abs/2606.00021 | published | READ BODY |
| H1.26 | Adaptive trigger + relaxed verification | Fang et al. | ReSpec, arXiv 2511.01282 | https://arxiv.org/abs/2511.01282 | published | READ BODY |
| H1.27 | CoT as draft + suffix cache | Valluri et al. | SSR, arXiv 2608.20359 | https://arxiv.org/abs/2608.20359 | published | READ BODY |
| H1.28 | n-gram drafters across 11 languages | Paudel, Ginn et al. | arXiv 2605.30580 | https://arxiv.org/abs/2605.30580 | published | READ BODY |

### H2. **Attempted-and-abandoned in n-gram / PLD** (verbatim)

| # | What was tried | Outcome | Verbatim | URL | Read |
|---|---|---|---|---|---|
| H2.1 | **llama.cpp n-gram lookup decoding — abandoned by its own author on a cost/benefit argument** | author's own negative conclusion | *"Since the current trend for models is going towards larger vocabulary sizes I think that it is **not worthwhile to invest more work into n-gram-based lookup decoding** unless the latency increase from an increase in batch size were to become extremely small."* / *"with Gemma 2 you actually get a **performance regression**"* / *"~50 previous runs on an RTX 4090 to sufficiently populate the dynamic lookup cache in order to break even. After ~100 previous runs the speedup is ~10%."* | https://github.com/ggml-org/llama.cpp/pull/8648 | READ BODY |
| H2.2 | **A planned deployment, closed NEGATIVE on measurement** | CLOSED NEGATIVE | *"Status: CLOSED NEGATIVE — keep the public recipe target-only"* / *"only 22 accepted tokens from 336 generated draft tokens … (6.55% reported acceptance)"* / *"Do not enable generic n-gram speculation in the Ornith guide."* Measured JSON: 113.000242 → **96.423720** tok/s. | https://raw.githubusercontent.com/steveseguin/b70-optimization-lab/main/experiments/ornith-15-b70/notes/2026-08-23-ornith35b-ngram-speculation-negative.md | READ BODY |
| H2.3 | ngram-mod stuck verification loop; mitigation PR closed unmerged | **the actual bug was never fixed** | *"This is WiP and a mitigation PR, not an actual fix."* / *"However, **the root cause of the loop is not yet clear.**"* | https://github.com/ggml-org/llama.cpp/pull/25819 | READ BODY |
| H2.4 | SGLang n-gram batch-threshold fallback | **closed unmerged** 2026-03-05, no maintainer comment visible | *"Speculative Decode often works well for relatively smaller batch sizes. **When the batch size is large, we want to run regular decode.**"* | https://github.com/sgl-project/sglang/pull/13376 | READ BODY |
| H2.5 | Sampling-penalty support for v1 n-gram | **auto-stale, then closed by author, unmerged** | bot: *"This pull request has been automatically marked as stale because it has not had any activity within 90 days."* | https://github.com/vllm-project/vllm/pull/18441 | READ BODY |
| H2.6 | **n-gram is Pareto-worse regardless of configuration** | `not planned` + stale | *"I have seen that **regardless of the configuration, the inference of the sample model n-gram model is Pareto worse than the inference without the n-gram model**"* / *"I could not make my experiments fly, despite significant effort."* | https://github.com/vllm-project/vllm/issues/16258 | READ BODY |
| H2.7 | vLLM n-gram CI accuracy gate failing | — | *"only 54 out of 100 times (54%), which is below the required 66% accuracy threshold"* / `AssertionError: assert 54 >= 66` | https://github.com/vllm-project/vllm/issues/35168 | READ BODY |
| H2.8 | HF transformers: PLD is **not** standard decoding | — | issue title/body | https://github.com/huggingface/transformers/issues/30448 | READ BODY |
| H2.9 | RAG multi-ngram · Qwen3 tool-call truncation · other n-gram asks | all `not planned` + stale | — | https://github.com/vllm-project/vllm/issues/6785 · https://github.com/vllm-project/vllm/issues/21307 · https://github.com/vllm-project/vllm/issues/16883 | READ BODY |
| H2.10 | SGLang n-gram issue closed inactive | — | — | https://github.com/sgl-project/sglang/issues/5365 | READ BODY |
| H2.11 | GPU n-gram async acceptance mismatch | PR `needs-rebase` | — | https://github.com/vllm-project/vllm/pull/44056 | READ BODY |

### H3. Papers stating n-gram/retrieval limits (verbatim)

| # | Finding, verbatim | URL | Read |
|---|---|---|---|
| H3.1 | *"SuffixDecoding alone is outperformed by EAGLE-2/3 and Token Recycling, as expected for less repetitive scenarios."* And on PLD: *"these methods speculate a fixed number of tokens irrespective of acceptance likelihood, leading to wasted computational resources"* | https://arxiv.org/html/2411.04975v3 | READ BODY |
| H3.2 | RACER-in-SGLang: *"history-only NGRAM becomes much less effective when reusable context patterns are sparse"* — ShareGPT, Qwen3.8-27B/A800: NGRAM 28.15 tok/s (1.02×) vs RACER 41.03 (1.48×) | https://github.com/sgl-project/sglang/pull/39211 | READ BODY |
| H3.3 | DReSD on REST: *"its effectiveness is limited due to the usage of short contexts and **exact string matching**."* | https://aclanthology.org/2025.findings-acl.1017/ | READ BODY |
| H3.4 | *"Retrieval-based methods (e.g., PLD, REST) rely on exact token matching and **fail completely when matching continuations do not exist in the context**"* | https://en.papernotes.org/ACL2026/llm_efficiency/racer_retrieval-augmented_contextual_rapid_speculative_decoding/ | READ BODY |
| H3.5 | *"capped at a 4-token match window (NGRAM_CACHE_MAX_NGRAM)… drafts stay short and get rejected."* / *"**Acceptance percentage alone does not rank a proposer.**"* | https://github.com/Mesh-LLM/mesh-llm/pull/1037 | READ BODY |
| H3.6 | AdaPLD: *"lexically anchored retrieval has limited recall under surface-form variation, and deterministic span copying can be brittle when the retrieved context does not uniquely determine the continuation."* | https://arxiv.org/abs/2606.05742 | READ BODY |
| H3.7 | SENSE: *"its potential is impeded by **rigid lexical dependencies**, rendering both retrieval and verification brittle to surface-level variations."* | https://arxiv.org/abs/2606.00021 | READ BODY |
| H3.8 | ReSpec: *"retrieval-enhanced methods like SAM-Decoding rely on heuristic switching strategies that often trigger **unnecessary retrievals**."* | https://arxiv.org/abs/2511.01282 | READ BODY |
| H3.9 | N-Grammys: *"the predicted next token of the base model is rarely the top prediction of these simple strategies, we observe that it is **often within their top-k predictions for small k**."* | https://arxiv.org/abs/2411.03786 | READ BODY |
| H3.10 | Multi-language: *"we propose weaker n-gram models as draft models; these provide **moderate** speed-ups due to their minuscule inference cost"* — i.e. n-gram is the fallback, not the winner | https://arxiv.org/abs/2605.30580 | READ BODY |
| H3.11 | Test-time scaling: *"simple n-gram-based methods effectively capture repetitive patterns, demonstrating unique potential in accelerating test-time scaling"* | https://arxiv.org/abs/2509.04474 | READ BODY |

### H4. Open n-gram items with verbatim asking

| # | Question | WHO | Verbatim | URL | Read |
|---|---|---|---|---|---|
| H4.1 | SGLang n-gram trie cannot take an external corpus and does not scale to long inputs | SGLang roadmap | *"**No external corpus support.** The trie is only populated from the current decoding session's output tokens. Ngram speculative decoding works best with a large reference corpus, but there is currently no mechanism to load one."* / *"**Insert path does not scale to long inputs.** It builds trie paths for almost every suffix, leading to **near-O(n²) memory growth**; when capacity is full, eviction can only remove leaf nodes and may fail."* Unchecked roadmap items: *"Expand SAM match length (currently still cap by max_trie_depth)"*, *"SAM for user request's input"*, *"Transport SAM across scheduler in multi-scheduler system (disagg, DP, etc)"* | https://github.com/sgl-project/sglang/issues/21052 | READ BODY |
| H4.2 | vLLM has **no global cross-request n-gram prompt-lookup cache** | hzeng2000, PR #44597 (Open, needs-rebase, conflicts) | *"The existing ngram proposer only scans the current request context. With prompt_lookup_cache_scope=\"global\", the proposer records prompt ngram continuations in a bounded process-local cache and can reuse them across requests. **The default remains local.**"* GitHub banner: *"This pull request has merge conflicts that must be resolved before it can be merged."* | https://github.com/vllm-project/vllm/pull/44597 | READ BODY |
| H4.3 | llama.cpp suffix decoding has **no global corpus tree** | PR #26283 (Open) | *"Currently, we build only the online tree only and no global corpus tree. vLLM optionally keeps the global cross-request cache (up to 10k past requests). **We implement the per-request/prompt tree only.**"* | https://github.com/ggml-org/llama.cpp/pull/26283 | READ BODY |
| H4.4 | **vLLM cannot combine MTP + n-gram** — `--speculative-config` takes one method | vLLM #46977 (Open) | *"Currently `--speculative-config` accepts a single method."* / *"**ngram is effectively free (zero inference cost, no VRAM overhead) and excels precisely in those repetitive ranges, but contributes nothing to base generation speed on its own.**"* | https://github.com/vllm-project/vllm/issues/46977 | READ BODY |
| H4.5 | n-gram corrupts tool calls, still open | vLLM #40875 (Open) | *"tool-call output is corrupted in ~50% of requests even on a stack with all known related fixes applied… Config-only workaround (no code changes): **set `prompt_lookup_min=8`**."* | https://github.com/vllm-project/vllm/issues/40875 | READ BODY |
| H4.6 | SGLang NGRAM argument validation gaps | SGLang #36495, #36352, #38129, #36500 | *"Neither the official parameter documentation nor the CLI help states that **capacity must be greater than maximum trie depth**, and the server accepts the combination at startup."* | https://github.com/sgl-project/sglang/issues/36495 · https://github.com/sgl-project/sglang/issues/36352 | READ BODY |
| H4.7 | SuffixDecoding authors name batch-level speculation control as future work | Oliaro et al., App. C | *"These are interesting and important directions for future work."* | https://arxiv.org/html/2411.04975v3 | READ BODY |
| H4.8 | Goose: batched serving and sampling-based verification remain open | Jin, Nguyen, Inoue | *"production-grade batched serving and sampling-based verification … remain future work"*; *"batched AR overtaking as batch size grows (crossover ≈ 4–16)"* | https://arxiv.org/html/2604.02047v2 | READ BODY |
| H4.9 | vLLM's own docs concede the ceiling | vLLM docs | *"simpler methods such as n-gram and suffix decoding provide **modest** speedups without increasing workload during peak traffic"* | https://docs.vllm.ai/en/latest/features/speculative_decoding/ | READ BODY |

### H5. n-gram hardware notes (corrects a possible over-read of section D)

**Binding constraints found for n-gram/PLD/suffix are TOPOLOGY and EVALUATION HARDWARE, not VRAM. No source read states a >96 GB requirement for n-gram/PLD/suffix drafting.**

| # | Item | URL | Read |
|---|---|---|---|
| H5.1 | SuffixDecoding e2e SWE-Bench: *"vLLM is deployed on **4 H100 GPUs** configured with 4-way tensor parallelism"*; cluster: *"single p5.48xlarge AWS instance equipped with **8 × NVIDIA H100 80G**"* | https://arxiv.org/html/2411.04975v3 | READ BODY |
| H5.2 | TRT-LLM n-gram study: *"Hardware: **8 × B200 GPUs** (Blackwell) … Tensor Parallel: 8"* — all its speedups | https://github.com/NVIDIA/TensorRT-LLM/blob/a4349b2cd9a521af85335a517722f90fc14d4f15/blogs/tech_blog/blog07_NGram_performance_Analysis_And_Auto_Enablement.html | READ BODY |
| H5.3 | Lookahead: *"4x with strong scaling on multiple GPUs"* (single-accelerator claim is only *"up to 1.8x on MT-bench"*) | https://arxiv.org/abs/2402.02057 | READ BODY |
| H5.4 | **Counter-evidence: single-GPU n-gram/suffix work is normal.** SGLang #36978: *"Tested with Qwen3-0.6B on one NVIDIA RTX 4090"*; SuffixDecoding: *"single H100 GPU with batch size 1"* | https://github.com/sgl-project/sglang/pull/36978 · https://arxiv.org/html/2411.04975v3 | READ BODY |
| H5.5 | ⚠️ **Absence of evidence, not a documented block:** Suffix decoding needs the out-of-tree CUDA dep `arctic-inference==0.1.1`; docs say *"CUDA Only"* and *"No DP Attention"*. **No source read states sm120 / RTX PRO 6000 support** — this is unverified, not ruled out. | https://raw.githubusercontent.com/vllm-project/vllm/v0.29.0/docs/features/speculative_decoding/suffix.md | READ BODY |
...
Their own full search logs (≈300 additional verbatim queries across web_search, GitHub HTML issue search, arXiv search/abs/HTML, OpenAlex, HuggingFace discussion JSON, and raw source retrieval) are preserved in their reports at:
- `spec_decode_heads_sweep.md` (heads sweep, 131 URLs, 373 lines)
- `specdec_sweep/REPORT.md` (draft-budget sweep, 55 closed + 23 open + 38 abandoned rows)
- `specdec_sweep/REPORT_ngram_pld_retrieval.md` (n-gram/PLD/retrieval sweep, 497 lines, 162-entry search log)
- `specdecode_sweep/specdecode_batching_quant_kvcache_sweep.md` (batching/quant/kv-cache sweep, 729 lines)
- `pa_out/FINAL_draft_models_sweep.md` (draft models, 508 lines)
- `pa_out/spec-decode-feature-interactions-sweep.md` + `pa_out/C_abandoned.md`, `DF_dflash_arch.md`, `E_data_cost.md` (systems integration, 493 + 356 + 217 + 305 lines)

### Engine-specific searches (verbatim, by engine)
**GitHub issue-search URLs:** `is:issue speculative is:closed reason:"not planned"` (vLLM and SGLang) · `is:pr speculative is:closed is:unmerged` (both) · `is:pr "spec dec"|eagle|mtp|dflash OR dspark is:unmerged` (vLLM) · `label:wontfix speculative` (0 results) · `speculative+deprecated+OR+deprecate`, `speculative+decoding+removed`, `prompt+lookup+deprecat`, `medusa`, `spec+decode+kv_cache_dtype`, `rejection+sampling+quantized`, `speculative+decoding+multi-node` (vLLM) · `speculative+remove`, `lookahead`, `fp8+kv+cache+speculative`, `speculative+8xH100` (SGLang) · `speculative+draft+wontfix`, `is:issue+medusa+speculator`, `is:issue+MLP+speculator`, `is:issue+"draft+model"+in:title`, `is:pr+is:closed+is:unmerged+spec+decode` (vLLM) · `is:issue+%22draft+model%22+in:title`, `speculative+deprecat` (SGLang) · SpecForge, speculators, llama.cpp, ollama, TGI, TensorRT-LLM equivalents.
**API-form:** `api.github.com/search/issues` with `reason:not_planned` across vLLM, SGLang, SpecForge, speculators, llama.cpp, ollama, transformers, TensorRT-LLM, TGI, mlx-lm.
**Forum:** `discuss.vllm.ai/search.json?q=` over `dynamic+speculative+decoding`, `num_speculative_tokens`, `acceptance+rate`, `speculation+length`, `adaptive+K`, `EAGLE+num_steps`, `draft+acceptance+batch`; topics 738, 1099, 2447, 2630, 499, 1096.
**Engines that returned nothing usable (recorded so they are not re-run blindly):** `export.arxiv.org/api/query` → HTTP 429 on every attempt from this sandbox; `arxiv.org/search/` HTML scrape → 0 parseable entries for several queries; `github.com/<repo>/tree/...` directory listings → repeated 60 s timeouts (individual blob/raw URLs work); `raw.githubusercontent.com/.../refs/heads/main/_posts/...` → HTTP 000 (the working form omits `refs/heads/`); direct `curl` to `huggingface.co` → HTTP 000 for several cards while `web_fetch` succeeded on the same URLs; DuckDuckGo HTML → 0 parsed results (throttled); `api.openalex.org` → usable, 9+ queries.
**Search-result negatives (recorded as findings):** literal `wontfix` on vLLM → **no results**; the `not planned` **label** on vLLM/SGLang/SpecForge/speculators → **no results**; `repo:ml-explore/mlx-lm is:issue reason:not_planned draft speculative` → **0 results**; "SpecTrain" → **no matching artifact found by any query**.
