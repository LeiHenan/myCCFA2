...
### 6.4 2026-09-15 续：用**因果裂缝扫描器**补筛投机地图 → 第 9 次 S3b（0 GPU·h）

上一轮把"**上游说了因果、但没做匹配对照**"确立为新的机会类型（FILTER 附录 A.6）。本轮用
`pipeline/tools/causal_gap_scan.py --map spec-decode-gap-map-2026-09-13.md` 对该地图机械筛选：**8 条候选**（其中 5 条是元叙述噪声）。
最好的一条是 **llama.cpp #23636**：*"MTP collapses at KV-slot boundaries"* ——
**ctx-size 只差 256 token，MTP 从 2.1×（AR 78%）掉到 1.04×（AR 0.6%）**，且被一句模板回复
（*"Follow the issue template"*，`not_planned`）关闭，报告者末句是 *"KV slot boundary root cause … remains open."*
—— 形状上是**离散崩塌 + 关闭理由非技术性**，池① 与因果裂缝同时成立。

**S3b 判定：🔴 已被占位**：llama.cpp **#23658**（#23636 的合规重开版）**CLOSED / COMPLETED**；
vLLM **#47602（OPEN）** 正推动**同一干预**（*"per-request (or at least length-bucketed) toggle/threshold for spec decode"*）⇒
与占位图**第 1 行**（草稿预算自适应）重合。

**⇒ 至此三条路径全部探尽**：
| 路径 | 状态 |
|---|---|
| **池①**（C 段理由过期） | KV 185 条 + 投机 C/G 段 → **幸存 0** |
| **因果裂缝**（我发明的机会类型） | 可达候选**已探尽**：KV 图 25 条里只有 1 条真可达（→ p22，实测上游**是对的**）；投机图 8 条里最好一条被 S3b 杀（→ 6.4）；其余属元叙述或不可达 |
| **池②**（从未被讨论过） | 三条线（引擎执行代价 / KV / 投机）**在跑** |
