...
- **杀因**：量级崩塌 + 低于噪声地板。**S3b 同时判 OCCUPIED**：FlashInfer **PR #4393 已 MERGED**（PCIe 无 NVLink 定制 all-reduce，12 KiB **5.3 µs vs NCCL 205.1 µs = 38.8×**），SGLang #34528（OPEN）在其上 TPOT **21.14 → 13.62 ms** ⇒ 该 fabric 上**已发货的答案是"消除延迟"，不是"重叠"**。
...
| R5 | **子代理的"范围判断"与它的检索结论一样需要回读原文复核** | 本目标 **3 次**：池① 范围误判（KV 185 其实已过筛）、"C1 不可测"的边界、numerical-drift 的 interim 假阳性 |
