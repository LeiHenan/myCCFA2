...
|---|---|
| 8 个给定方向 | **全部裁决完毕**（§1） |
| 候选池 ①（失败理由依赖已变约束） | **耗尽**：KV 185（上轮）· INFERENCE 263（本轮）· 投机 C+G（上轮），合计 **0 幸存** |
| 候选池 ②（从未被讨论过） | **耗尽**：在**本机 `/root/ccfa_venv` 的 vLLM 0.29.0 树**上做过（扫 3952 条 TODO/not-supported） |
| 候选池 ③（因果裂缝） | 最佳实例 **S-B 被其零卡可分离性闸门杀掉**；`causal_gap_scan.py` 机械筛选噪声大，不足以直接立项（工具改造方向见 §5.1） |
| 地图自身硬件杀因审计 | **完成**：11 条实测 → 6 HOLD / 4 FALSE / 1 UNVERIFIABLE，**0 条过 §0.6 的 2% 门槛** ⇒ 不发 §3 句 |
| 自建候选 | **5 条全部被事前冻结的判据杀掉**（C1 阻塞 · p30 · p28→p35 · S-B · C258） |
| 交付物 | 本文件 + `notes/{OCCUPANCY_LEDGER,NEW_SERVER_ENV_2026-09-15,KILLREASON_AUDIT_2026-09-15,SB_SEPARABILITY_2026-09-15,POOL1_ENGINE_UNLOCKED_2026-09-15}.md` + `results/p35-fixed-axis/{SUMMARY.md,raw/}` |
| 仓库 | 已提交并推送（`origin/main`，0 领先） |
| AutoDL 服务器 | **已关闭**（5 次探测 `Connection refused`） |
| schoolserver | **保持运行**（按用户 2026-09-15 的指示；该指示只针对旧 AutoDL 机） |

**预算**：本目标累计 **≈1.1 GPU·h**（上限 8/轮）。

**最终判断（不变的二分）**：
> 在 ~700 个地图格 + 263 + 185 + 8 个方向 + 5 条自建候选上一致成立的是
> **「够大的都被占着，没被占的都不够大。」**
> 池①/B 段产出"够大但已占位"，池②产出"空闲但够不着 §0.6 的 2% 门槛"。
> **这不是搜索失败，而是本工作区约束（单卡 sm120 / dense 4B / 无多节点 / 无 SGLang）下可发布空间的边界测量结果。**
