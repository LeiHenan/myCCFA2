...
### 9.12 S-B 的**零卡可分离性闸门**：`SEPARABLE FORM FITS ⇒ KILL`（未花 1 秒 GPU）

`notes/SB_SEPARABILITY_2026-09-15.md`（390 行）。抽到 ~90 个逐条引用的数值格，其中承重的是
#54691 的 **15 个 break-even 接受长度阈值**（真 3 ctx × 5 batch 网格：2k 1.47/1.42/1.55/1.63/1.74 ·
32k 1.91/2.09/2.82/3.77/4.94 · 131k 3.51/4.53/6.79/10.20/14.59）。

**判决依据（精确算术，无拟合）**：
1. 已发表的 pay/no-pay 矩阵被**一个 2 有效参数的可分离评分 15/15 复现**：`pay ⟺ B·ctx^1.568 < 6.3e7`；
   自由形式 `f(B)+g(ctx)`（7 参数）与"2 拐点阶梯"（3 参数）**也都拟合**。
2. **那 15 个符号全部是推断、不是测量**——该断言的核心标量 `a` **在任何格上都没被测过**。
3. 全库只有 **7 个坐标**有*测量*的 pay/no-pay 标签，其中只有 **2 个**是"投机 vs 不投机"；
   **没有任何一格同时有 >1 个 batch 值与 >1 个 ctx 值**。

⇒ **S-B 按原定口径不成立，杀掉。**（`notes/FILTER_3AXIS.md` §0.6 的直接成果：零卡上界杀在开卡之前。）

**残留但未获许可的更窄主张**（记录下来，避免下一轮重新发现）：
· **成本函数形式错误**：#54691 自己的**实测**分解显示，固定 B=1 时 **draft 项 1.24→7.21 ms（5.8×）**
  而 target step 只动 **1.097×** ⇒ 若按 `F(ctx)+K·M(batch)`，`F(2k)/F(131k)` 需为 5.34，**实测是 0.91**。
  这与 #54749 §2 的"是系数问题、不是函数形式问题"**结论相反**。
· **但**同一作者后来的固定 batch 诊断（#49986 评论 11）**未能复现**上下文依赖（95× ctx 范围内平，−0.10 pp；
  batch 携带 +15.02 pp），并明确警告"并发随上下文变化的扫描会把 batch 效应折进 ctx 读数"。
· ⇒ 这是**标定/测量方法**问题，且带着作者自己的"我的测量方法有问题"的告诫 ⇒ **同样不许可开卡**。
· 若要翻转此杀：只需测 #54749 自称"写好未跑"的 2×2 四格的 **可达接受长度 a**（非 break-even），
  现有 harness、无需新代码；**伪证条件：若 a(32k,B=1) ≤ 1.91，整个矛盾自行消解。**

### 9.13 池① 全量过筛完毕（263 格）＋ **我对子代理范围判断的更正**

**子代理交付** `notes/POOL1_ENGINE_UNLOCKED_2026-09-15.md`：逐格过筛 **263** 个 `REASON-MAY-HAVE-EXPIRED`，
深读**已装 0.29.0 源码核验 20 格**（另顺手 6 格）。**幸存 0。**

**子代理的范围判断有误，我已核对原文更正**：它声称"KV 185 / 投机 ~60 那批带引文的放弃格从未被真正过筛"。
实际 `notes/GOAL3_CLOSE_2026-09-15.md:438` 明写 **"KV 地图 185 条带引文的放弃格 → 幸存 0（§1.5/§1.8）｜已耗尽"**，
`:492` 又写 **"池① 两条线（KV 185 条、投机 C+G）全部过筛，幸存 0"**。
⇒ 那批**上一轮已过筛**；子代理把"没有 `REASON-MAY-HAVE-EXPIRED` 标签"误当成"从未过筛"。
（**教训**：子代理的"范围修正"与它的检索结论一样需要复核——这是本目标第 3 次"子代理的断言需回读原文"。）

⇒ **池① 现在三张图全部耗尽**：KV 185（上轮）· INFERENCE 263（本轮）· 投机 C+G（上轮），**合计 0 幸存**。

**池② 也已耗尽**：`KV_NEVER_DISCUSSED.md` 方法头写明它就是在**这台机器、这个 `/root/ccfa_venv` 的 vLLM 0.29.0 树**
上做的（扫描 3952 条 TODO/not-supported，2026-09-14），target 同为 Qwen3-4B。

**本轮杀因统计（子代理）**：不是占位，而是
· **~120 格**的 R2 条件落在**本机没有的配置**上（量化 / MoE / MLA / 多卡 / ROCm / SGLang / 训练）；
· **~90 格**的 R1 约束在 0.29.0 源码里**逐字仍在**。
**唯一过了 §0.6 算术门槛的候选 C258**（`enable_qk_norm_rope_fusion`）：
3 次发射→1 次，净省 2×1.0–1.5 µs×36 层 = **72–108 µs/步**；权重读下界 t_base≈5.3 ms（8.0e9 B ÷ 1.52 TB/s），
现实 7–12 ms ⇒ **0.6–1.5% < 2% ⇒ KILLED**。且上游在 H100 上测到的是**负号**（issue #34391 正文无任何数字，已读）。
**新占位者（地图未记）**：vLLM PR **#47979**「SM120 PCIe serving stack」状态 **`DRAFT`**。

### 9.14 已开启的最后一个未查方向：**审计地图自身的"杀因"**

依据：本轮子代理**从已装源码/二进制挖出三条与地图矛盾的一手事实**
（sm120 **有** split-KV：`_vllm_fa2_C.abi3.so` 导出 561 个 split 符号且 Qwen3-4B 的 32:8 GQA decode 形状可走到；
sm120 **有** FlashInfer XQA decode + `AttentionCGSupport=UNIFORM_BATCH`，既有日志显示捕获 35 张 FULL 图；
`IS_QUANTIZED=IS_DENSE=False` **全局硬编码**于 `vllm/config/vllm.py:129-134`）。
⇒ 地图可能**以错误理由杀过格子**。这与本目标已两次踩到的"筛子本身错了"同属一类
（§9.1 硬件档写死、§9.9 引擎归因错机），且前两次都真的掉出了东西，故值得一查。
审计范围：只查**关于"这台硬件/引擎能做什么"的事实性杀因**，不重开"被占位"与"模型族不匹配"两类（那是合法杀）。
