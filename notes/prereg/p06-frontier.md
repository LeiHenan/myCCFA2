...
| **T1 反转探针**（主假设） | 半天–1 天 | `depth{1,3,5} × γ{1,3,7} × ctx{4k,128k} × bs{1}` ≈ **18 格** × ≥3 次重复 | **存在配置反转** ⇒ 主假设存活；**完全无反转** ⇒ 杀 |
...
**硬件（2026-09-12 按实测配置更正）**：T1 = `ctx{4k,128k} × bs{1}` 单卡。以 `Qwen3-4B` + `Qwen3-4B-speculator.dflash2` 为例：**144 KiB/token** ⇒ 128k 的 KV = **18.0 GiB（FP16）/ 9.0 GiB（FP8）**，权重 ≈8 GiB、drafter ≈1 GiB。⇒ **T0 只需 ≥16 GB；T1 需单卡 ≥24 GB**：24 GB（4090）跑 128k 格需 `--kv-cache-dtype fp8`（≈18 GiB，偏紧）或把 ctx 收到 32k；**32 GB（5090）最省心**；48/80 GB 宽裕（并可带 185k 格，但那属于 W2 的全网格）。**T0/T1 均不需要多卡**。所有对照条件必须**同 KV dtype**，且论文须注明 GPU 型号。详见 `notes/p06-toolchain-check.md` §5。
