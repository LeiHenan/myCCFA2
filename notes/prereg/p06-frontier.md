...
| 2026-09-12 | **S1：同一预登记下的 bs 扩展（数据采集前登记）** —— 原 T1 只测 `bs=1`；按本 prereg「bs 扩展条件」条款补测 **4k 臂 `bs{1,8,32}` × depth{1,3,5} × γ{3,7} × REPS=5**（90 格）与 **32k 臂 `bs{1,2,4}` × 同前 × REPS=3**（54 格）。同时修正两个混淆：加 `--disable-shuffle`（否则 rep 间 prompt 顺序+prefix cache 状态不同，实测极差 10.6%）、prompt 数 8→32。**判据先行登记**：深 drafter 在所有 bs 与 γ 上都不劣 ⇒ 该子空间 No-Go 成立；任一处浅层反超且超噪声 ⇒ 重做策略级 T2。**待补**：S2 换 ≥128k target（需 ≥48 GB 显存）、S3 换 block 更大的族测长 γ、S4 跑 shipped-adapter 基线 | **数据采集前**（S1 尚未开跑） |
| 2026-09-13 | **A5 预登记 + A1-32k 收窄（数据采集前；设计审计的结论，见 `notes/audit-p06-design-2026-09-13.md`）** —— ① **A1/S1 实际只跑了 γ{3,7}，而本 prereg §量 登记的 γ 因子是 `{1,3,5,7}`** ⇒ **γ=1/5 从未测过**，且缺失的 γ→1 角落恰是"drafter 代价占比 ∝ 1/γ"这一结构预测唯一可能反转的地方（4k 机制分解实测：d1→d5 每步代价仅 +5.9%，接受长度却 +111%，逐位置接受率 d1 = `0.28 0.06 0.00×5` 是**悬崖而非权衡**）。**A5 = 补格**：4k，`bs{1,8,32} × depth{1,3,5} × γ{1,2,5} × 32 prompts × 5 reps`（135 格），**其余参数与 A1 逐字相同**（`GPU_UTIL=0.45 / MAXSEQS=32 / eager / KV=bfloat16 / REQS=32 / OUTLEN=128`）⇒ 与 A1 的 γ{3,7} 行可直接合并分析。**γ=2 为本轮新增档**（原登记为 {1,3,5,7}），登记时点=数据采集前，动机=定位"d5 优势消失"的临界 γ。**证伪性预测（先登记）**：**P1** d5 的相对优势随 γ 下降**单调收缩**，且在 γ=1 处 ≤ **+10%**（允许为负）；**P2** 接受率悬崖在 γ=1 处仍在（d1 的 k≥1 位置接受率 ≈ 0）；**P3** γ=1 处 d5 的每步代价相对增幅 **大于** γ=7 处。**判定**：P1 成立 ⇒ 反转只存在于"已 ship 控制器本就把 γ 压到 0–1"的角落（vLLM `num_speculative_tokens_per_batch_size`）⇒ 判据⑥ 的增量上限 = 0，`#6` 按预登记判死并归档；**P1 被反驳**（γ=1 处 d5 仍 ≥ +20%）⇒ 本审计的代价结构论证有误，须重开 32k 全网格。② **A1-32k 收窄为 bs=8 单点**：`ctx=32768 × bs{8} × depth{1,3,5} × γ{3,7} × 5 reps`（30 格），`GPU_UTIL≈0.72 / MAXSEQS=8 / REQS=8`。理由：32k 单请求 KV = 4.5 GiB ⇒ bs=32 需 144 GB > 96 GB **物理不可行**（原 `bs{1,2,4}` 已隐含承认）；而在中高并发下 d5 的 KV 比 d1 少 16%（serve log：242,403 vs 203,980 tokens）**必然触发驱逐/排队**，按**判据⑦ 那类反超不可采纳为机制证据**。⇒ **新增可采纳前置条件（先登记）：该格 `vllm:num_preemptions_total == 0` 且 serve log 的 `GPU KV cache size` ≥ 需求（8×32k = 262,144 tokens）**；不满足则该格**只作现象记录、不计入机制判定** | **数据采集前**（A5 与收窄版 A1-32k 均尚未开跑；γ=1/5 为本 prereg 原登记档，γ=2 为新增档，均在此登记） |
| 2026-09-13 | **A1/A3/A4 预登记（数据采集前，接手 2026-09-12 复核）** —— 承接 decision #61（bs 轴上深度最优翻转）与 #62（占位核查）。**A1**：4k 全网格 `bs{1,8,16,32} × depth{1,3,5} × γ{3,7} × 32 prompts × 5 reps`（120 格），统一 `GPU_UTIL=0.45 / --max-num-seqs 32 / --enforce-eager / KV=bfloat16`，在 96 GB 机上跑（含此前在 32 GB 机上 OOM 的 γ=7 半臂）。**判据③（决定性）**：比较 `argmax_depth(bs)` 在 γ=3 与 γ=7 下是否不同 —— 不同且差 > 2×重复噪声 ⇒ **深度与长度耦合，二维联合分配问题成立**；若两 γ 下最优深度相同且恒定 ⇒ 只支持"按 bs 选一个固定深度"，**机制层弱化**。**A3（KV 中立控制）**：在 `bs=16`（需求 65k tokens，而三档可用 KV 均 ≥130k ⇒ 无深度受 KV 限制）看深度排序是否仍反转 —— 仍反转 ⇒ 归因于**计算代价**；消失 ⇒ 归因于**显存挤占**（叙事退化）。**A4**：跑 vLLM `num_speculative_tokens_per_batch_size` 得到 shipped-adapter 基线（判据⑥的比较基准）。**所有判据在此登记，先于 2026-09-13 的数据采集** | **数据采集前** |
