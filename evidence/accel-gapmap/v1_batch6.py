#!/usr/bin/env python3
"""V1 pass 6 (final mechanical pass): key regex tolerant of a parenthetical in the KEY,
but never mutating the VALUE.  Unescapes literal \\n / \\" before relocation.
"""
...
out = open(os.path.join(HERE, "v1_checkall6.jsonl"), "w", encoding="utf-8")
        ...
        for k, v in fl:
            ...
            for s in QUOTED.findall("\n".join(v)):
                if len(s) >= 40:
                    cands.append((k, s))
        for tag, q in cands:
            r = best_of(urls, unesc(q)) if urls else {"verdict": "NO-URL"}
            rec = {"id": b["id"], "file": fn, "section": b["section"], "tag": tag,
                   "urls": urls, "quote": q[:600], "verdict": r.get("verdict"),
                   "ratio": r.get("ratio"), "missing_tail": r.get("missing_tail"),
                   "url_checked": r.get("url"), "per_url": r.get("per_url")}
            out.write(json.dumps(rec, ensure_ascii=False) + "\n")
            out.flush()
            print(f"{rec['verdict']:18s} {str(rec.get('ratio')):6s} {fn:6s} {b['id']:8s} "
                  f"{tag[:20]:20s} {rec['quote'][:70]!r}", flush=True)
out.close()
