#!/usr/bin/env python3
"""V1 pass 4 (definitive mechanical pass).

For every block in S1..S4:
  * primary quote field (QUOTE / ASKING_QUOTE / STATED_REASON / HARDWARE_QUOTE) -> first occurrence only
  * PLUS every double-quoted span >= 40 chars found in any NOTE/SECOND/... field
Each candidate is checked against EVERY url listed in the block's URL: field.
Writes v1_checkall4.jsonl.
"""
import json, os, re, sys
HERE = os.path.dirname(os.path.abspath(__file__))
sys.path.insert(0, HERE)
import verify as V

FILES = sys.argv[1:] or ["S1.md", "S2.md", "S3.md", "S4.md"]
BLOCK = re.compile(r"^###\s+([ABCDE][0-9.]*?)\s*\|\s*(CLOSED|OPEN|ABANDONED|HARDWARE-RULED-OUT|NEVER-DISCUSSED)\s*(.*)$")
KEY = re.compile(r"^([A-Z][A-Z0-9 _()/\.\->+]*?):\s?(.*)$")
RANK = {"MATCH": 3, "MATCH-FRAGMENTS": 2, "NEAR": 1}


def strip_parens(s):
    return re.sub(r"\s*\([^)]*\)", "", s)


def norm_key(k):
    return re.sub(r"\s*\(.*?\)\s*", "", k).strip().replace(" ", "_")


def blocks(fn):
    cur = None
    last = None
    for raw in open(os.path.join(HERE, fn), encoding="utf-8", errors="replace"):
        line = raw.rstrip("\n")
        m = BLOCK.match(line)
        if m:
            if cur:
                yield cur
            cur = {"id": m.group(1), "section": m.group(2), "sub": m.group(3).strip(" ()"),
                   "file": fn, "fields": []}
            last = None
            continue
        if cur is None:
            continue
        probe = strip_parens(line)
        f = KEY.match(probe) if probe[:1].isupper() else None
        if f:
            last = [norm_key(f.group(1)), [f.group(2).strip()]]
            cur["fields"].append(last)
        elif last and line.strip() and line.strip() != "---":
            last[1].append(line.strip())
    if cur:
        yield cur


def get(fl, *names):
    """first field whose key == name (in order of names), joined; None if absent"""
    for n in names:
        for k, v in fl:
            if k == n:
                return "\n".join(v).strip()
    return None


def get_prefix(fl, *prefixes):
    for p in prefixes:
        for k, v in fl:
            if k.startswith(p):
                return "\n".join(v).strip()
    return None


def primary(sec, fl):
    if sec == "OPEN":
        return get_prefix(fl, "ASKING_QUOTE")
    if sec == "ABANDONED":
        return get_prefix(fl, "STATED_REASON")
    if sec == "HARDWARE-RULED-OUT":
        return get_prefix(fl, "HARDWARE_QUOTE")
    if sec == "NEVER-DISCUSSED":
        return get_prefix(fl, "GAP")
    return get_prefix(fl, "QUOTE")


QUOTED = re.compile(r'"([^"]{40,})"', re.S)
urlre = re.compile(r"https?://[^\s;,]+")
out = open(os.path.join(HERE, "v1_checkall4.jsonl"), "w", encoding="utf-8")

for fn in FILES:
    for b in blocks(fn):
        fl = b["fields"]
        urls = urlre.findall(get_prefix(fl, "URL") or "")
        cands = []
        pq = primary(b["section"], fl)
        if pq:
            cands.append(("PRIMARY", pq))
        for k, v in fl:
            if k in ("QUESTION", "WHO", "ARTIFACT", "URL", "STATUS", "EVIDENCE", "WHO_IS_STILL_ASKING"):
                continue
            if k.startswith(("QUOTE", "ASKING_QUOTE", "STATED_REASON", "HARDWARE_QUOTE")):
                continue
            txt = "\n".join(v)
            for s in QUOTED.findall(txt):
                if len(s) >= 40:
                    cands.append((k, s))
        if not cands and pq is None:
            out.write(json.dumps({"id": b["id"], "file": fn, "section": b["section"],
                                  "verdict": "NO-QUOTE", "urls": urls}, ensure_ascii=False) + "\n")
            print(f"NO-QUOTE {'':6s} {fn:6s} {b['id']}", flush=True)
            continue
        for tag, q in cands:
            rec = {"id": b["id"], "file": fn, "section": b["section"], "tag": tag,
                   "urls": urls, "quote": q[:400]}
            if not urls:
                rec["verdict"] = "NO-URL"
            else:
                best = {"verdict": "MISMATCH", "ratio": 0.0}
                per = []
                for u in urls:
                    try:
                        r = V.check(u, q)
                    except Exception as e:
                        r = {"verdict": "FETCH-ERROR", "err": str(e)[:100]}
                    per.append({"url": u[-48:], "v": r.get("verdict"), "ratio": r.get("ratio")})
                    if RANK.get(r.get("verdict"), 0) > RANK.get(best.get("verdict"), 0) or (
                            RANK.get(r.get("verdict"), 0) == RANK.get(best.get("verdict"), 0)
                            and (r.get("ratio") or 0) > (best.get("ratio") or 0)):
                        best = dict(r)
                        best["url"] = u
                rec.update({"verdict": best.get("verdict"), "ratio": best.get("ratio"),
                            "missing_tail": best.get("missing_tail"),
                            "url_checked": best.get("url"), "per_url": per})
            out.write(json.dumps(rec, ensure_ascii=False) + "\n")
            out.flush()
            print(f"{rec['verdict']:18s} {str(rec.get('ratio')):6s} {fn:6s} {b['id']:8s} "
                  f"{tag[:22]:22s} {rec['quote'][:70]!r}", flush=True)
out.close()
