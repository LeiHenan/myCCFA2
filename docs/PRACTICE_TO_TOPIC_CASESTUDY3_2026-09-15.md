...
| P4 | **丢弃的 softmax 质量**作为**输出漂移**的代理：同一 n 下 TV 差 **3–31×**（n=1024 为 **12.6×**），两个模式下 TV 对质量**非单调**，`TV/丢弃质量` 跨度 oracle **440×**、randn **1323×** | `results/p28-sparse-verify-drift/…/summary.md` |
| P5 | 同一代理量**双向失效**：质量 **0.60** 处 `window` 的 TV=**0.025**（远小于质量）、`random` 的 TV=**0.933**（约 **1.5 倍**质量）⇒ 作为代理量**可以差 37×**；`TV = 丢弃尾质量` 只对 top-k 成立（**构造性恒等**，非普适） | `results/p31-p33-structure/README.md` |
| P6 | **prefill 时间（token 数 × 32.6 µs）**作为"前缀缓存端到端价值"的代理：命中率 75.3% 时真 prefill 只剩 **6.8 ms** 而 TTFT p50 = **31.4 ms**，真实到达率下未命中 prefill 只占 e2e 的 **1.2%**（而同一代理量先前给 4.8–9.1% ⇒ **高估约 10×**） | `results/p43-replay/SUMMARY.md` |
| P7 | **每步字节 ÷ 带宽**作为 **step 时间**的代理：L=313 下字节只涨 **36%** 而步时间涨 **2.58×**，batch ≳ **28** 之后该代理量失明 | `results/p39-costmodel/SUMMARY.md` |
...
### 4.2b 而且这不是孤例——是**成组异常**（§4.1 要的正是这个）

按 §4.1「单个异常不是课题，成组异常背后的不变量才是」，我对**全部结果目录**做了一次存在性取证
（`git ls-files` + `find`，零成本）：

| 被具名引用的产物 | 取证结果 |
|---|---|
| `notes/BOTTLENECK_INVENTORY_2026-09-15.md`（案例研究 #1 的头号资产） | `git log --all --diff-filter=A -- '*BOTTLENECK*'` **只命中加入该引用的提交本身** ⇒ **从未入库** |
| `results/p35-fixed-axis/raw/L512.jsonl`（其 `SUMMARY` 里"候选死亡"判定的**原始证据**，**360 格**，`analyze.py` 的全部比值/CI 都由它算出） | `git ls-files \| grep -c L512` = **0**；`find` = **0** ⇒ **不在库**（其 `raw/README.md` 仍在库并明确点名它） |
| `results/<probe>/<date>/summary.md`（`results/README.md` 写明"**必填**"） | **39 个结果目录里 17 个没有**（44%） |

**不变量**：*这个仓库最被引以为据的那类产物——"可复核的资产"——恰恰是最容易缺失的。*
引用它的文档在库、判决在库，**而据以判决的数据不在库**。

**同一个病的第三个变体（仓库自己记着）**：`.gitignore:16-18` 自述其白名单曾经**过窄**：
`t1.csv`/`ridge.csv`/`T2-verdict.md`/`VERDICT.md` 被**静默忽略**（`git add -A` 不报错、`git status` 也看不出来，
**必须用 `git ls-files` 读回**）；且实例上的 **54 个 `bench.json` 已因同步 `rm -rf` 被删 ⇒ 逐次重复数据永久丢失**。
⇒ 缺失不是偶发：**有一次是规则写的、有一次是同步命令删的、有两次是被引用却从未入库的。**

**⚠️ 我自己在这个数字上犯了一次 §7.2 的错（留档）**：本节初稿写的是"**25/39（64%）**"，
成因是 `find -name "summary.md"` **大小写敏感**，而该仓库同时存在 `summary.md`（16 份）与 `SUMMARY.md`（8 份）两种写法。
改用 `-iname` 后正确值为 **17/39（44%）**。**这正是本条要找的那类错误**：
产物在库、判决在库，**而我读它的方式本身是错的**——且它不会被数据推翻，只会被换一种读法推翻。
（附带发现：`results/README.md` 只规定 `summary.md`，但 **8 个目录用了 `SUMMARY.md`** ⇒ 约定与实现不一致，
这正是"按名字找文件"这类检查最容易静默漏掉的地方。）

**但按方法论 §3 闸 1，这条不构成候选**："只改自己的（是缺陷，修掉即可）"。
⇒ 它**是缺陷**，产出形态是**修复 + 一条方法论条文**（已在 v3 §8 落地），不是课题。
把这条写在这里，是为了让它可被复核，而不是为了给它立题。
