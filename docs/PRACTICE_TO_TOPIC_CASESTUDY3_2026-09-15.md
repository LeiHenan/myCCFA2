...
| P8 | **接受率**作为**加速比**的代理，**上游已有四处独立报告**（见下），与 P1 合起来说明"高接受率 ≠ 更快"是**公开发表过的** | `notes/DIRECTION_MAP_3AXIS.md:186-190` |

**P8 的四处上游实例（这条把"接受率失明"从候选降为已占位）**：
[llama.cpp #23533](https://github.com/ggml-org/llama.cpp/issues/23533) *"MTP is **~21% slower** … **despite 100% draft accuracy**"*；
[vLLM #15025](https://github.com/vllm-project/vllm/issues/15025) *"acceptance rate is **very high** … **consistent performance drop ~30%**"*；
[vLLM #16258](https://github.com/vllm-project/vllm/issues/16258) *"regardless of the configuration … **Pareto worse**"*；
一份 2026-08-23 的部署记录 **113.00 → 96.42 tok/s**（"CLOSED NEGATIVE"，**22 accepted / 336 drafted = 6.55%**）。
⇒ 我这轮最看好的候选（α 的"接受率失明"支线）**同样被上游占位**。
...
### 4.2c 同一类病的第二层：**综合层（地图/台账/结题文档）内部对不上账**

我按 §7.1 对该仓库的**综合层**做了一次零成本复算（只做加减），抽 4 条核对：

| # | 文档里的说法 | 复算 | 结论 |
|---|---|---|---|
| 1 | `notes/GOAL3_CLOSE_2026-09-15.md:520`「**336 条挂 S3b 标签**（198 NO-ACTIVE-WORK-FOUND / 123 OCCUPIED / 91 SOLVED）」 | 198+123+91 = **412** | **对不上（差 76）** |
| 2 | `:423`「E1–E5 共 **88 行**」→ `:516`「最终 E 段是 **63 条**（证伪 23 条、移除 24 条）」 | 88−23−24 = **41** | **对不上（差 22）** |
| 3 | 案例研究 #1 `:107`「**13 格**瓶颈清单：8 格已榨干（4 占位 + 4 实测），4 格标未知」 | 8+4 = **12** | **对不上（差 1 格未归类）** |
| 4 | `notes/DELIVERY_B_GOAL_65274f4c.md:5`「**18 条**自建机制，0 存活」 | 表格编号行 = **18** | ✅ **一致**（撤销此项指控） |
| 5 | `notes/KILLREASON_AUDIT_2026-09-15.md:36` 一行内：D2/D3/D4/D5 分项写 16/16/7/3，却称小计 **31**、总计 **~45** | 16+16+7+3 = **42** | **对不上（同一行内两处：≠31 且 ≠~45）** |
| 6 | 同文件 `:41`「**180** 条 device-count/VRAM/fabric/storage kills 被排除」；`:40-41` 称在范围 **≈28** | 三张地图 130+94+45 = **269**；排除 108+87+31 = **226** ⇒ 在范围应为 **43** | **对不上（180≠226；269−226=43≠≈28）** |

**⇒ 6 条里 5 条对不上、1 条经复核撤销。** 第 4 条是我按"委派审计的指控必须自己复核"的纪律撤销的
（与案例研究 #2 §6 的两次撤销同一做法）。

**与 4.2b 合起来的不变量**：*该仓库的**实验层**远比它的**综合层**可靠。*
实验层有预登记、有冻结判据、有原始 JSON、有回算；而综合层（地图、台账、结题文档、案例研究）
在**纯加减**层面就已经对不上账，并且**引用了不存在的资产**。
两者叠加就是：**判决可复算，而判决的索引不可复算。**

**定性（为什么仍然不是课题）**：按 §3 闸 1，这仍是"只改自己的"⇒ **缺陷**。
但它给出了一条比 4.2b 更可执行的修复顺序：**先修综合层的账，再补实验层的 summary**——
因为综合层正是读者据以判断"哪里还有空间"的那一层。

### 4.2d 唯一接近"可执行产出"的东西：**仓库自己排好的 8 格待判队列，从未被做**

`notes/DIRECTION_MAP_3AXIS.md:158`（原文）：

> **待判（按"先便宜后贵"排序）**：**B108 → B116 → B114 → B109 → B107 → C-DSD → C-NVFP4-KV → B-draft_model**。

其中 B108（前缀缓存 + batch-invariant）在该文件 `:147` 被标为**"新增头号待判"**，且注明
**"只差一次分钟级实验 + 零卡源码核"**（因为 p15/p16 全部是在"前缀缓存关闭"下做的）。

⇒ **这是整个现场里最接近 to-do 清单的东西**：优先级已排、成本已估、判据已定，**但从未执行**。
它不是新课题（旧地图里已有编号与定义），而是**旧队列的未清项**。
按 §8 停止规则，它们属于"下一次分析会改变决定"的少数残留——**值得清，但不构成立题**。
...
>
> **补充（子代理回报后）**：我本轮最看好的那条——"接受率不是加速比的代理"——
> 也在上游有**四处独立报告**（llama.cpp #23533 "21% slower despite 100% draft accuracy" 等）⇒ **同样被占位**。
> 至此**三个候选、以及它们的支线，全部被占位**。
>
> 确定的产出是两件**缺陷**（不是课题）：**入库缺口**（2 个被具名引用的资产从未入仓 + 17/39 目录缺必填 summary）
> 与**综合层的账对不上**（抽查 6 条、**5 条不一致**、1 条撤销），以及由此得出的**方法论 v3 两条修订**。
> 唯一接近可执行产出的是**仓库自己排好却从未执行的 8 格待判队列**（B108 → … → B-draft_model）。
